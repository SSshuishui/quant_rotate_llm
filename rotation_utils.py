...
def rotate_model(model, args, logger):
...
@torch.inference_mode()
def rotate_model_global_only(model, args, logger):
...
def add_qk_rotation_wrapper_after_function_call_in_forward(module, function_name, *args, **kwargs):
    ...
    import monkeypatch
    import functools
    attr_name = f"{function_name}_qk_rotation_wrapper"  # apply_rotary_pos_emb_qk_rotation_wrapper
