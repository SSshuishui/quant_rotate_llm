...
from utils import fuse_norm_utils, hadamard_utils, quant_utils, utils, data_utils


def quarot_train(model, args, logger):
    from eval_utils import rotation_utils, gptq_utils

    if args.rotate:
        logger.info("====== Rotate ======")
        fuse_norm_utils.fuse_layer_norms_quarot(model)
        rotation_utils.rotate_model_quarot(model, args, logger)
    ...
    if args.w_bits < 16:
        ...
        if args.load_qmodel_path: # Load Quantized Rotated Model
            ...
            quantizers = gptq_utils.gptq_fwrd(model, trainloader, DEV, args, logger)
    ...
    if args.a_bits < 16 or args.v_bits < 16:
        ...
        qlayers =quant_utils.find_qlayers(model, layers=[ActQuantWrapper])
    ...
    if args.k_bits < 16:
        if args.k_pre_rope:
            ...
            rope_function_name = "apply_rotary_pos_emb"  # apply_rotary_pos_emb
...
def spinquant_train(args, model, logger):
    ...
    if args.rotate:
        logger.info("====== Rotate ======")
        # Rotate the weights
        fuse_norm_utils.fuse_layer_norms_spinquant(model)
