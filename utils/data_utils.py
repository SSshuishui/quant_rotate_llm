...
def get_wikitext2(nsamples, seed, seqlen, model, hf_token, eval_mode=False):
    ...
    if eval_mode:
        testdata = datasets.load_dataset('wikitext', 'wikitext-2-raw-v1', split='test', cache_dir="/data/huggingface_cache/datasets")
        testenc = tokenizer("\n\n".join(testdata['text']), return_tensors='pt')
        return testenc
    else:
        traindata = datasets.load_dataset('wikitext', 'wikitext-2-raw-v1', split='train', cache_dir="/data/huggingface_cache/datasets")
...
def get_ptb_new(nsamples, seed, seqlen, model, hf_token, eval_mode=False):
    ...
    if eval_mode:
        testdata = datasets.load_dataset('ptb_text_only', 'penn_treebank', split='test', cache_dir="/data/huggingface_cache/datasets")
        testenc = tokenizer(" ".join(testdata['sentence']), return_tensors='pt')
        return testenc
    else:
        traindata = datasets.load_dataset('ptb_text_only', 'penn_treebank', split='train', cache_dir="/data/huggingface_cache/datasets")
